...
{
    /**
     * Ассоциативный массив, хранящий информацию о языке, контроллере, экшне и атрибутах, переданый в url,
     * либо заполненный умолчаниями или элементами из $_SESSION, если в url информации нет
     * @var array
     */
    private $lcaa = [
        'language'   => '',
    ...
    public function start($config){
        try {
            ...
            /**
             * Получаем ассоциативный массив с языком, контроллером, экшном и атрибутами из роутера
             */
            //$this->lcaa = Router::arr_FromUrl(); //простой роутер
    ...
    private function mvc ($controller, $action, $attributes = null) {
         ...
         * то будет выполнено действие index, если оно отсутствует, то будет ошибка
         */
        call_user_func_array([$controller_obj, $this->lcaa['action'] . 'Action'], $attributes);
    }

    /**
     * Метод которой выводит ошибку
     * @param $errno integer
     * @param $errstr string
     * @param $file string
     * @param $line integer
